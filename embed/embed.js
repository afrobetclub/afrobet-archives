...
function renderDailyChart(rows){
  ...
  rows.forEach(row => {
    ...
    years.push(row.date.getFullYear());
  });

  const firstIndexByYear = {};
  years.forEach((year, index) => {
    if(firstIndexByYear[year] === undefined){
  ...
  const yearLabelPlugin = {
    ...
    afterDraw(chart){
      ...
      ctx.fillStyle = "#726D65";
      ctx.textAlign = "center";
      ctx.textBaseline = "top";

      Object.keys(firstIndexByYear).forEach(year => {
        const index = firstIndexByYear[year];
        const px = x.getPixelForValue(index);

        if(px >= chartArea.left && px <= chartArea.right){
          ctx.fillText(year, px, chartArea.bottom + 14);
        }
      });
  ...
  chartInstance = new Chart(ctx, {
    ...
    options:{
      ...
      animation:false,
      layout:{padding:{top:34,right:28,bottom:34,left:0}},
      plugins:{
    ...
    plugins:[lastPointLabelPlugin, yearLabelPlugin]
  });
}
